	...
	function login(){
		...
		$.ajax({
			...
			if (jsonObj == false) {
				...
				//alert("登录成功！");
				window.location.href="${pageContext.request.contextPath }/cashier.jsp"
			}
		});
	}
</script>
...
<script type="text/javascript">
	function convertCanvasToImage() {
		...
		//alert("666");
		$.ajaxFileUpload({
			url : 'FaceServlet?type=faceIdentify',
			...
			success : function(data, status) {
				...
				//alert("后台返回数据："+data.toString());
				//var result = data.toString().substr(5, data.length - 11);
				//var json = $.parseJSON(result);
				/* 因为后台回显数据带有<pre>标签，所以只能通过解析字符串的方式的提取出json对象 */
				/* 后台回显实例（Firefox浏览器）：<pre>{"result":[{"uid":"User_1001","scores":[97.004425048828],"group_id":"group_01","user_info":"覃璠科"}],"log_id":2748389701072022,"result_num":1}</pre> */
				/* 通过判断json对象是否存在"result"键来确定回传信息（是否验证成功） */
				// 这个提取方法Firefox浏览器有效，其他浏览器如谷歌，在<pre>标签中还有属性，请自行根据所使用的浏览器处理
				//if ("result" in json) {
					// 验证成功
					//if (parseFloat(json.result[0].scores) >= 80) {
						// 相似度评分大于等于80
						//alert("用户ID：" + json.result[0].uid + "\n用户信息："
								//+ json.result[0].user_info + "\n相似度评分："
								//+ json.result[0].scores + "\n组ID："
								//+ json.result[0].group_id);
					//} else {
						//alert("你是谁？");
					//}
				//} else {
					/*后台回显实例：<pre>{"log_id":2920864716072110,"error_msg":"face not found","error_code":216402}</pre> */
					// 验证失败
		...
		<div id="setp_quicklogin">
			...
			<div id="btn">
				...
				<button type="button" class="btn btn-info"
					onclick="convertCanvasToImage()">登录</button>
				<!-- <button onclick="convertCanvasToImage()">验证</button> -->
			</div>
